...
public class GMatrix implements java.io.Serializable, Cloneable {
	 ...
	 * The array values are copied in one row at a time in row major fashion. The array
	 * should be at least nRow*nCol in length. Note that because row and column numbering
	 * begins with zero, nRow and nCol will be one larger than the maximum possible matrix
	 * index values.
	 *
	 * @param nRow
	 *        number of rows in this matrix.
	...
	/**
	 * Computes the outer product of the two vectors; multiplies the the first vector by
	 * the transpose of the second vector and places the matrix result into this matrix.
	 * This matrix must be be as big or bigger than getSize(v1)xgetSize(v2).
	 *
	 * @param v1
	...
	/**
	 * Sets the value of this matrix to the matrix difference of itself and matrix m1
	 * (this = this - m1).
	 *
	 * @param m1
	 *        the other matrix
	...
	/**
	 * Sets the value of this matrix to the matrix difference of matrices m1 and m2 (this
	 * = m1 - m2).
	 *
	 * @param m1
	 *        the first matrix
	...
	/**
	 * Subtracts this matrix from the identity matrix and puts the values back into this
	 * (this = I - this).
	 */
	...
	/**
	 * Inverts matrix m1 and places the new values into this matrix. Matrix m1 is not
	 * modified.
	 *
	 * @param m1
	...
	/**
	 * Copies a sub-matrix derived from this matrix into the target matrix. The upper left
	 * of the sub-matrix is located at (rowSource, colSource); the lower right of the
	 * sub-matrix is located at (lastRowSource,lastColSource). The sub-matrix is copied
	 * into the the target matrix starting at (rowDest, colDest).
	 *
	 * @param rowSource
	 ...
	 * @param rowDest
	 *        the top-most row of the position of the copied sub-matrix within the target
	 *        matrix
	 * @param colDest
	 *        the left-most column of the position of the copied sub-matrix within the
	 *        target matrix
	 * @param target
	 ...
	 * values will be lost. If the size is decreased, only those data values whose matrix
	 ...
	 * values are copied in one row at a time, in row major fashion. The array should be
	 * at least equal in length to the number of matrix rows times the number of matrix
	...
	public final void set(Matrix3f m1) {
		...
		if ((this.nCol < 3) || (this.nRow < 3)) { // expand matrix if too small
			this.nCol = 3;
			this.nRow = 3;
			this.values = new double[this.nRow][this.nCol];
		}
		...
		for (i = 3; i < this.nRow; i++) { // pad rest or matrix with zeros
	...
	public final void set(Matrix3d m1) {
		...
		for (int i = 3; i < this.nRow; i++) { // pad rest or matrix with zeros
	...
	public final void set(Matrix4f m1) {
		...
		for (int i = 4; i < this.nRow; i++) { // pad rest or matrix with zeros
	...
	public final void set(GMatrix m1) {
		...
		for (i = m1.nRow; i < this.nRow; i++) { // pad rest or matrix with zeros
	...
	/**
	 * Places the values in the this GMatrix into the matrix m1; m1 should be at least as
	 * large as this GMatrix.
	 *
	 * @param m1
	 *        The matrix that will hold the new values
	 */
	...
	public final void transpose() {
		...
		if (this.nRow != this.nCol) {
			...
			this.nRow = this.nCol;
			this.nCol = i;
			tmp = new double[this.nRow][this.nCol];
			for (i = 0; i < this.nRow; i++) {
				for (j = 0; j < this.nCol; j++) {
	 ...
	 * GMatrix objects with identical data values (i.e., GMatrix.equals returns true) will
	 * return the same hash number. Two GMatrix objects with different data members may
	 ...
	 * distance is equal to MAX[i=0,1,2, . . .n ; j=0,1,2, . . .n ; abs(this.m(i,j) -
	 * m1.m(i,j)]
	 *
	 * @param m1
	 ...
	 * U*W*transpose(V); and returns the rank of this matrix; the values of U,W,V are all
	 * overwritten. Note that the matrix V is output as V, and not transpose(V). If this
	 * matrix is mxn, then U is mxm, W is a diagonal matrix that is mxn, and V is nxn.
	 * Using the notation W = diag(w), then the inverse of this matrix is: inverse(this) =
	 * V*diag(1/w)*tranpose(U), where diag(1/w) is the same matrix as W except that the
	 * reciprocal of each of the diagonal components is used.
	 *
	 * @param U
	...
	/**
	 * LU Decomposition: this matrix must be a square matrix and the LU GMatrix parameter
	 * must be the same size as this matrix. The matrix LU will be overwritten as the
	 * combination of a lower diagonal and upper diagonal matrix decompostion of this
	 * matrix; the diagonal elements of L (unity) are not stored. The GVector parameter
	 * records the row permutation effected by the partial pivoting, and is used as a
	 * parameter to the GVector method LUDBackSolve to solve sets of linear equations.
	 * This method returns +/- 1 depending on whether the number of row interchanges was
	 ...
	 * @return +-1 depending on whether the number of row interchanges was even or odd
	 *         respectively
	 */
	...
	/**
	 * Given a nxn array "matrix0", this function replaces it with the LU decomposition of
	 * a row-wise permutation of itself. The input parameters are "matrix0" and "dim". The
	 * array "matrix0" is also an output parameter. The vector "row_perm[]" is an output
	 * parameter that contains the row permutations resulting from partial pivoting. The
	 * output parameter "even_row_xchg" is 1 when the number of row exchanges is even, or
	 * -1 otherwise. Assumes data type is always double.
	 *
	 ...
	 * come from luDecompostion and do not change here. The parameter "matrix2" is a set
	 * of column vectors assembled into a nxn matrix of floating-point values. The
